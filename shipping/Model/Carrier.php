...
//                dd($this->checkoutSession->getData());
    ...
    public function getCheapestValue($arrCheapest, $serviceDefault, $enableServiceAP, $serviceAP, $enableServiceADD, $serviceADD, $countryLower)
    {
        ...
        // }
        if ($serviceDefault == '0' && $enableServiceADD == '1' && !empty($serviceADD)) {
            $returnData = $serviceADD;
        }
        return $returnData;
    }
        ...
        if (!empty($listDeliveryRatesFromDatas)) {
            ...
            foreach ($listDeliveryRatesFromDatas as $deliveryRate) {
                ...
                if ($serviceId != $deliveryRate[\UPS\Shipping\Helper\ConstantCarrier::SERVICE_ID]) {
                    ...
                    $saveDeliveryRate['flatCountries'] = $flatCountries;
                    $saveDeliveryRate['flatRules'] = $flatRules;
                    $orderValues = [];
                    $orderRates = [];
                    $flatCountries = [];
                    $flatRules = [];
                    if ($countListDeliveryRatesFromDatas == 1 && isset($listDeliveryRatesFromDatas[0][\UPS\Shipping\Helper\ConstantCarrier::SERVICE_ID])) {
                        $serviceId = $listDeliveryRatesFromDatas[0][\UPS\Shipping\Helper\ConstantCarrier::SERVICE_ID];
                        $saveDeliveryRate['orderValues'][] = $deliveryRate['min_order_value'];
                        $saveDeliveryRate[\UPS\Shipping\Helper\ConstantCarrier::ORDERRATES][] = $deliveryRate['delivery_rate'];
                        if (isset($deliveryRate['flat_country']) && !empty($deliveryRate['flat_country']) && isset($deliveryRate['flat_rule']) && !empty($deliveryRate['flat_rule'])) {
                            $saveDeliveryRate['flatCountries'][] = $deliveryRate['flat_country'];
                            $saveDeliveryRate['flatRules'][] = $deliveryRate['flat_rule'];
                        }
                    }
                    ...
                    if (isset($deliveryRate['flat_country']) && !empty($deliveryRate['flat_country']) && isset($deliveryRate['flat_rule']) && !empty($deliveryRate['flat_rule'])) {
                        $flatCountries[] = $deliveryRate['flat_country'];
                        $flatRules[] = $deliveryRate['flat_rule'];
                    }
            ...
            if ($shippingType = 'ADD' && $responseRateAPI && isset($responseRateAPI['Fault']['detail']['Errors']['ErrorDetail']['PrimaryErrorCode']['Code'])) {
                ...
                foreach ($listShipServices as $shippingServiceItem) {
                    ...
                    if (!empty($serviceRate)) {
                        $allRatedShipment = [];
                        if (isset($responseRateAPI['RateResponse']['RatedShipment'][0])) {
                            $allRatedShipment = $responseRateAPI['RateResponse']['RatedShipment'];
                        } else {
                            $allRatedShipment[] = $responseRateAPI['RateResponse']['RatedShipment'];
                        }
                        foreach ($allRatedShipment as $ratedShipment) {
                            $apiRateCode = $ratedShipment['Service']['Code'];
                            $apiSatDeliFlg = isset($ratedShipment['TimeInTransit']['ServiceSummary']['SaturdayDelivery']) ? $ratedShipment['TimeInTransit']['ServiceSummary']['SaturdayDelivery'] : "";
                            ...
                            // if (isset($ratedShipment['ItemizedCharges'][0])) {
                            //     foreach ($ratedShipment['ItemizedCharges'] as $item) {
                            //         $apiMonetaryValue += $item['MonetaryValue'];
                            //     }
                            // } else {
                            //     $apiMonetaryValue += $ratedShipment['ItemizedCharges']['MonetaryValue'];
                            // }

                            $apiTimeInTransit = isset($ratedShipment['TimeInTransit']) ? $ratedShipment['TimeInTransit'] : [];
                            ...
                            if (intval($shippingServiceItem['rate_code']) === intval($apiRateCode)) {
                                if ((strpos($shippingServiceItem['service_key'], 'SAT_DELI') !== false && intval($apiSatDeliFlg) == 1)
                                    ...
                                    $des_con = isset($params[\UPS\Shipping\Helper\ConstantCarrier::COUNTRY_ID]) ? $params[\UPS\Shipping\Helper\ConstantCarrier::COUNTRY_ID] : "";
                                    if (isset($serviceRate['flatCountries']) && !empty($serviceRate['flatCountries']) && isset($serviceRate['flatRules']) && !empty($serviceRate['flatRules'])) {
                                        if (array_search($des_con, $serviceRate['flatCountries']) === false && array_search("ALL", $serviceRate['flatCountries']) === false) {
                                            continue;
                                        }
                                    }
                                    // get time from TinT API and by shipping code
                                    $shippingServiceFee = $this->_getDeliveryRates($serviceRate, $orderTotalPrice, $shippingServiceItem, $apiMonetaryValue, $des_con);
     ...
     * @return array $data
     */
    private function _getDeliveryRates($serviceRate, $orderTotalPrice, $shippingServiceItem, $rateShipmentMonetaryValue, $desCountry)
    {
        $shippingServiceFee = 0;
        if (strtoupper($serviceRate[\UPS\Shipping\Helper\ConstantCarrier::RATE_TYPE]) == 'FLAT_RATE') { //Flat rates
            if (isset($serviceRate[\UPS\Shipping\Helper\ConstantCarrier::ORDERRATES]) && isset($serviceRate['flatCountries']) && !empty($serviceRate['flatCountries']) && isset($serviceRate['flatRules']) && !empty($serviceRate['flatRules'])) {
                //Country based Flat
                $arrDeliveryRates = $serviceRate[\UPS\Shipping\Helper\ConstantCarrier::ORDERRATES];
                $arrDeliveryValues = $serviceRate['orderValues'];
                $arrDeliveryCountries = $serviceRate['flatCountries'];
                $arrDeliveryRules = $serviceRate['flatRules'];
                ...
                if (!empty($listPackageInfo)) {
                    foreach ($listPackageInfo as $pack_info) {
                        $orderTotalWeight += (isset($pack_info['weight']) && !empty($pack_info['weight'])) ? $pack_info['weight'] : 0;
                    }
                }
                $lastWeight = 0;
                $lastValue = 0;
                foreach ($arrDeliveryCountries as $key => $countryId) {
            ...
            } elseif (isset($serviceRate[\UPS\Shipping\Helper\ConstantCarrier::ORDERRATES])) {  //Non-country based flat (old versions)
                $arrDeliveryRates = $serviceRate[\UPS\Shipping\Helper\ConstantCarrier::ORDERRATES];
                $arrDeliveryValues = $serviceRate['orderValues'];
                $defaultCurrencyCode = $this->storeManager->getStore()->getDefaultCurrencyCode();
                $orderTotalPrice = $this->convertPrice($orderTotalPrice, $defaultCurrencyCode);
    ...
    private function _callRateAPI($accessorials, $params, $shippingService = [], $orderTotalPrice = 0, $cuttime = '', $rateType = 'SHOP', $shippingType = 'AP', $currencyCode = 'USD', $typeRate = \UPS\Shipping\Helper\ConstantCarrier::ESHOPPER, $firstLocation = [], $idAccount = '')
    {
        ...
        $arrPackageDimension = [];
        if (!empty($listPackageInfo)) {
            foreach ($listPackageInfo as $l_k => $package) {
                $arrPackageDimension[$l_k] = [
                    'PackageWeight' => [
                ...
                if (isset($package['length']) && isset($package['width']) && isset($package['height'])) {
                    $arrPackageDimension[$l_k]['Dimensions'] = [
                        \UPS\Shipping\Helper\ConstantCarrier::UNITOFMEASUREMENT => [
