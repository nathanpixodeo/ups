require([
    "jquery"
        ...
        $('#resetAccUsrPwd').click(function() {
            if($('#resetAcc').is(':visible'))
            {
                $('#resetIcon').html("▾");
                $('#resetAccUsr').removeAttr('data-validate');
                $('#resetAccPwd').removeAttr('data-validate');

                $('#AccountName').attr('data-validate', "{'required': true, 'validate-rule': ''}");
                $('#AddressLine1').attr('data-validate',"{'required': true, 'validate-rule': ''}");
                $('#PostalCode').attr('data-validate', "{'required': true, 'validate-rule': ''}");
                $('#AccountCity').attr('data-validate', "{'required': true, 'validate-rule': ''}");
                $('#PhoneNumber').attr('data-validate', "{'required': true, 'validate-rule': ''}");
                $('.accountWithOut90Days_show input').each(function() {
                    $(this).attr('data-validate', $(this).attr('data-validate-hidden'));
                });
            } else {
                $('#resetIcon').html("▴");
                $('#resetAccUsr').attr('data-validate', "{'required': true, 'validate-rule': ''}");
                $('#resetAccPwd').attr('data-validate', "{'required': true, 'validate-rule': ''}");

                $('#AccountName').removeAttr('data-validate');
                $('#AddressLine1').removeAttr('data-validate');
                $('#PostalCode').removeAttr('data-validate');
                $('#AccountCity').removeAttr('data-validate');
                $('#PhoneNumber').removeAttr('data-validate');
                $('.accountWithOut90Days_show input').removeAttr('data-validate');
            }
            $('#resetAcc').slideToggle(500);
        });
