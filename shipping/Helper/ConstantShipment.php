<?php
/**
 * ConstantShipment file
 *
 * @category  UPS_Shipping
 * @package   UPS_Shipping
 * @author    United Parcel Service of America, Inc. <noreply@ups.com>
 * @copyright 2019 United Parcel Service of America, Inc., all rights reserved
 * @license   This work is Licensed under the License and Data Service Terms available
 * at: https://www.ups.com/assets/resources/media/ups-license-and-data-service-terms.pdf
 * @link      https://www.ups.com/pl/en/services/technology-integration/ecommerce-plugins.page
 */
namespace UPS\Shipping\Helper;

/**
 * ConstantShipment class
 *
 * @category  UPS_Shipping
 * @package   UPS_Shipping
 * @author    United Parcel Service of America, Inc. <noreply@ups.com>
 * @copyright 2019 United Parcel Service of America, Inc., all rights reserved
 * @license   This work is Licensed under the License and Data Service Terms available
 * at: https://www.ups.com/assets/resources/media/ups-license-and-data-service-terms.pdf
 * @link      https://www.ups.com/pl/en/services/technology-integration/ecommerce-plugins.page
 */
class ConstantShipment
{
    const PAGE_ID = 'page_id';
    const SHIPMENT_ID = 'shipment_id';
    const MIN_WIDTH = 'min_width';
    const TRACKING_NUMBER = 'tracking_number';
    const ORDER_ID = 'order_id';
    const CREATED_AT_DATE = 'created_at_date';
    const CREATED_AT_TIME = 'created_at_time';
    const DELIVERY_ADDRESS = 'delivery_address';
    const ESTIMATED_SHIPPING_FEE = 'estimated_shipping_fee';
    const SERVICE_TYPE = 'service_type';
    const ATTENTIONNAME = "AttentionName";
    const PHONE = "Phone";
    const NUMBER = 'Number';
    const ADDRESS = "Address";
    const ADDRESSLINE = "AddressLine";
    const STATEPROVINCECODE = "StateProvinceCode";
    const POSTALCODE = "PostalCode";
    const COUNTRYCODE = "CountryCode";
    const FIRSTNAME = 'firstname';
    const LASTNAME = 'lastname';
    const DESCRIPTION = "Description";
    const AP_NAME = 'ap_name';
    const AP_ADDRESS1 = 'ap_address1';
    const AP_CITY = 'ap_city';
    const AP_POSTCODE = 'ap_postcode';
    const ACCESSORIAL_SERVICE = 'accessorial_service';
    const ACCESSORIALS = "accessorials";
    const COUNTRY = 'country';
    const ADDRESS_1 = 'address_1';
    const ADDRESS_2 = 'address_2';
    const ADDRESS_3 = 'address_3';
    const ENTITY_ID = 'entity_id';
    const SHIPPING_SERVICE = 'shipping_service';
    const ADDRESS1 = 'address1';
    const ADDRESS2 = 'address2';
    const ADDRESS3 = 'address3';
    const STATE = 'state';
    const POSTCODE = 'postcode';
    const STYLE_BR = '<br/>';
    const SHIPMENT = 'shipment';
}
